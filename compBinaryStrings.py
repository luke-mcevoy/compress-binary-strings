...
# we need math.log and math.ceil (the ceiling function)
...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        # the current run is still goin
        elif s[0] == curVal:
            return lORL(s[1:], [], curCount + 1, s[0])
        elif s[0] != curVal:
            return [curCount] + lORL(s[1:], [], 1, s[0]) 
        # return # TO DO (mysolution is about 6 lines
...
def findRunBits(s):
    List = listOfRunLengths(s)
    red = sum(List)
    average = red/len(List)
    n = math.ceil(math.log(average,2)) + 1
    runBits = min(n,7)
    return runBits
'''
...
def test():
    assert uncompress(compress(L0)) == L0
    assert uncompress(compress(L1)) == L1
    assert uncompress(compress(L2)) == L2
    assert uncompress(compress(L3)) == L3
    assert uncompress(compress(L4)) == L4
    assert uncompress(compress(L5)) == L5
    assert uncompress(compress(L6)) == L6
    return "No Errors"
